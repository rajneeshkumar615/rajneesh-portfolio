...
export const projects = [
  {
    ...
    name: "CodePulse AI",
    year: "2025",
    tagline: "AI-powered GitHub repository intelligence platform — six-stage analysis pipeline, six-dimension engineering score, AI chat.",
    description:
      "A deployed TurboRepo monorepo that turns any GitHub repository into a full engineering intelligence report. A six-stage pipeline — GitHub import, branch sync, repository scan, context building, AI evaluation, and report generation — feeds a dashboard that scores maintainability, security, performance, architecture, documentation, and testing, tracks trend deltas across every re-analysis, and answers questions through a repository-aware AI chat assistant.",
    stack: ["Next.js", "React", "TypeScript", "Node.js", "Express.js", "Prisma", "PostgreSQL", "JWT", "OpenRouter", "GitHub API", "TurboRepo"],
    highlights: [
      "Six-stage analysis pipeline — GitHub import, branch sync, repository scan, context building, AI evaluation, report generation — with per-stage status tracked and surfaced in the UI",
      "Six-dimension engineering score (maintainability, security, performance, architecture, documentation, testing) rendered as a health matrix, radar chart, and heatmap, with per-dimension trend deltas against the previous run",
      "Deterministic metrics engine computes every score; the AI layer (OpenRouter, via the OpenAI SDK) only explains and contextualizes what's already been measured",
      "Repository-aware AI chat assistant, architecture-flow diagrams, and a combined risk / technical-debt / deployment-readiness assessment per repository",
      "Reports export in PDF, JSON, or HTML — PDF for sharing, JSON for piping into other tools, HTML for a standalone viewable report",
      "Full analysis history stored per repository — every re-run diffed against the immediately preceding one, not a rolling average",
    ],
    screenshots: ["Engineering Dashboard", "Repository Health Matrix", "AI Repository Assistant"],
    link: "/projects/codepulse-ai",
    repo: "https://github.com/rajneeshkumar615/codepulse-ai",
    live: "https://codepulse-ai-web.vercel.app/",
  },
  {
    id: "zarrin-blogs",
    name: "Zarrin Blogs",
...
> = {
  "codepulse-ai": {
    overview:
      "CodePulse AI turns any GitHub repository into a full engineering intelligence report. A six-stage pipeline — GitHub import, branch sync, repository scan, context building, AI evaluation, and report generation — feeds a dashboard that scores six engineering dimensions, tracks trend deltas across every re-analysis, assesses production risk and deployment readiness, and answers follow-up questions through a repository-aware AI chat assistant.",
    architecture: [
      { layer: "Next.js Client", detail: "Dashboard, health matrix, radar/heatmap charts, repository explorer, and PDF/JSON/HTML report export — TypeScript, Tailwind, Framer Motion" },
      { layer: "Express API", detail: "JWT-secured TypeScript REST layer orchestrating the six-stage analysis pipeline, with Prisma-backed data access" },
      { layer: "Repository Scanner", detail: "GitHub API (Octokit) — branch sync, file-tree walk, and framework/language/tooling detection" },
      { layer: "Metrics Engine", detail: "Deterministic scoring across maintainability, security, performance, architecture, documentation, and testing" },
      { layer: "AI Analysis Layer", detail: "OpenRouter, via the OpenAI SDK, generates the executive summary, risk assessment, and chat responses — grounded in the metrics engine's output, never inventing scores" },
      { layer: "PostgreSQL / Prisma", detail: "Repository metadata and full per-run analysis history, so every re-analysis has a real previous score to diff against" },
    ],
    apiDesign: [
      "TurboRepo workspace separating apps/web (Next.js dashboard) and apps/api (Express) with shared types",
      "Pipeline-stage endpoints — import, sync, scan, context build, AI evaluate, report — with per-stage status persisted so progress is visible and a failed stage doesn't require re-running the whole analysis",
      "A context-builder layer assembles repository structure, detected stack, and prior analysis history before every AI call",
      "The chat endpoint reuses the same analysis context as the dashboard, so assistant answers stay grounded in the actual scan instead of the model's general knowledge",
    ],
    challenges: [
      {
        problem: "Letting the LLM compute engineering scores directly produced inconsistent, unverifiable numbers across repeated analyses of the same repository.",
        solution:
          "Split the pipeline so a deterministic metrics engine scores all six dimensions, and the AI layer's only job is to explain and contextualize numbers it's handed — never generate them.",
      },
      {
        problem: "Comparing a new analysis against the previous run required an honest baseline, not a rolling average that would blur what actually changed.",
        solution:
          "Stored full per-dimension scores for every analysis run in Postgres and diffed the latest against the immediately preceding run, so the trend deltas shown on the dashboard reflect exactly what changed.",
      },
      {
        problem: "Large repositories blew past usable context windows and drove up token costs on every analysis.",
        solution:
          "Added truncation caps in the file-reading layer and reworked prompt construction in the context builder to cut token usage while preserving the signal the model needs.",
      },
    ],
    performance: [
      "Truncation caps on file reads keep large-repository analysis within usable context limits",
      "Prompt engineering pass reduced token usage per analysis without losing analysis quality",
      "Per-stage pipeline status persisted in Postgres, so a failed stage can retry without re-running the full six-stage analysis",
    ],
    lessons: [
      "Keep AI as an explanation layer over deterministic computation, not the source of truth for metrics",
      "Monorepo tooling (TurboRepo) pays off once a frontend and backend need to evolve together with shared types",
      "Trend tracking is only honest if you store every run's full scores — you can't diff against history you didn't keep",
    ],
  },
  "staynest": {
    overview:
...
export const skills = {
  ...
  Frontend: ["React.js", "Next.js", "Redux", "Context API", "HTML5", "CSS3", "Tailwind CSS", "Bootstrap", "Framer Motion"],
  Backend: ["Node.js", "Express.js", "REST APIs", "JWT", "Socket.io", "Redis", "Prisma"],
...
> = {
  ...
  "React.js":    { color: "#61DAFB", glow: "rgba(97,218,251,0.35)",  category: "Frontend",      proficiency: "Primary frontend framework across all projects" },
  "Next.js":     { color: "#ffffff", glow: "rgba(255,255,255,0.18)", category: "Frontend",      proficiency: "App Router + TypeScript for CodePulse AI's dashboard client" },
  "Framer Motion":{ color: "#bb4df5", glow: "rgba(187,77,245,0.30)", category: "Frontend",      proficiency: "Motion and micro-interactions across dashboard UI" },
  ...
  Prisma:        { color: "#16A394", glow: "rgba(22,163,148,0.30)",  category: "Backend",       proficiency: "Schema and query layer over PostgreSQL for CodePulse AI" },
...
export const dashboardMetrics = [
  ...
  { id: "projects",    label: "Full-Stack Projects",  value: 3,   suffix: "",  group: "Experience" },
...
> = {
  "codepulse-ai": {
    liveUrl: "https://codepulse-ai-web.vercel.app/",
    repoUrl: "https://github.com/rajneeshkumar615/codepulse-ai",
    deploymentStack: ["Vercel (Web)", "Render (API)", "Prisma", "PostgreSQL", "TurboRepo"],
    environmentSetup: [
      "Environment variables (database URL, JWT secret, OpenRouter key) managed per environment, never committed to source control",
      "Prisma migrations applied against a managed PostgreSQL instance ahead of each deploy",
      "Web app (apps/web) deployed to Vercel, API service (apps/api) deployed separately to Render within the same TurboRepo workspace",
    ],
    hostingInfrastructure: [
      { layer: "Frontend", provider: "Vercel — Next.js dashboard and case-study pages" },
      { layer: "API",      provider: "Render — Node.js/Express service" },
      { layer: "Database", provider: "PostgreSQL — Prisma-managed schema and analysis history" },
    ],
  },
  staynest: {
...
export const projectSystems: Record<string, { name: string; detail: string }[]> = {
  "codepulse-ai": [
    { name: "Six-stage analysis pipeline", detail: "GitHub import, branch sync, repository scan, context building, AI evaluation, and report generation — each stage tracked with its own completion status." },
    { name: "Deterministic engineering scoring", detail: "Maintainability, security, performance, architecture, documentation, and testing are scored directly from repository data, then diffed against the previous run for trend deltas." },
    { name: "AI repository analysis", detail: "OpenRouter, via the OpenAI SDK, generates the executive summary, architecture intelligence, and risk assessment — explaining the metrics engine's output, never generating the scores itself." },
    { name: "Repository-aware AI chat", detail: "A chat assistant answers architecture, security, performance, and refactor questions grounded in the same analysis context as the dashboard." },
    { name: "Risk & deployment readiness", detail: "Aggregates technical debt and highest-risk items into a single production-readiness assessment, with a prioritized engineering roadmap per repository." },
    { name: "Multi-format report export", detail: "Full engineering reports — summary, metrics, AI analysis, roadmap, charts — export to PDF, JSON, or HTML, so a report can be shared, archived, or piped into another tool outside the dashboard." },
  ],
  staynest: [
...
export const architectureGallery: DiagramSpec[] = [
  {
    id: "codepulse-ai-pipeline",
    title: "CodePulse AI Analysis Pipeline",
    category: "Backend",
    steps: [
      { label: "GitHub Repository", detail: "Repository imported from GitHub" },
      { label: "Repository Sync",   detail: "Branches and metadata synchronized" },
      { label: "Repository Scanner",detail: "Static analysis and file inspection" },
      { label: "Context Builder",  detail: "Assembles engineering context for AI" },
      { label: "AI Intelligence",  detail: "OpenRouter evaluates repository quality" },
      { label: "Engineering Report",detail: "Final intelligence report generated" },
    ],
  },
  {
    id: "codepulse-ai-architecture",
    title: "CodePulse AI Architecture",
    category: "System",
    steps: [
      { label: "Next.js Client",    detail: "Dashboard, repo explorer, PDF/JSON/HTML export" },
      { label: "Express API",       detail: "JWT-secured TypeScript REST layer" },
      { label: "GitHub Scanner",    detail: "Octokit — repo structure & file trees" },
      { label: "Metrics Engine",    detail: "Deterministic score, complexity, debt" },
      { label: "OpenRouter AI",     detail: "Explains metrics via OpenAI SDK" },
      { label: "PostgreSQL/Prisma", detail: "Analysis history & repository data" },
    ],
  },
  {
