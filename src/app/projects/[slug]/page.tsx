...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const archIdPrefix: Record<string, string> = {
    staynest: "staynest",
    "zarrin-blogs": "zarrin",
    "codepulse-ai": "codepulse-ai",
  };
  const archDiagram = architectureGallery.find((d) =>
    d.id.startsWith(archIdPrefix[slug] ?? slug)
  );
  const jwtDiagram  = architectureGallery.find((d) => d.id === "jwt-flow");
  const usesJwtDiagram = project.id === "zarrin-blogs" || project.id === "codepulse-ai";
